...
const initialOptions = {
    labels: ["generic", "fuzzers", "reconnaissance","2"],
    colors: ["#4318FF", "#6AD2FF", "#FFAA33","#FF55AA"],
  };
  ...
  const fetchHistoryData = async () => {
    try {
      ...
      response.data.forEach(packet => {
        ...
        states[state]=(states[state]*response.data.length/100 + 1)/response.data.length*100;}
      });
  
      const pieChartData = Object.values(states);
