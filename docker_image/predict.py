...
def transform_to_dataframe(row):
    ohe_service = pickle.load(open(r'env\files\ohe_service.pkl','rb'))
    ...
    return df
def ann_predict(df):
    model_ann=tf.keras.models.load_model(r"env\files\ann_model.h5")
    y_pred = model_ann.predict(df)
    y_pred_classe = np.argmax(y_pred, axis=1)
    dic={'0':'exploits','1':'fuzzers','2':'generic','3':'other','4':'reconnaissance'}
    return dic[str(y_pred_classe[0])]
...
def prediction(row):
    ...
    model=lgb.Booster(model_file=r'\env\files\lightgbm_model.txt')
    p=model.predict(df, num_iteration=model.best_iteration)
    if(p[0]>0.5):
        return ann_predict(df)
    else:
        return "normal"
